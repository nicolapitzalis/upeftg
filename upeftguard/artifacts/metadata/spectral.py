...
from ...contracts.spectral import layer_identifier_for_block_name
from ...utilities.core.manifest import _resolve_adapter_path_for_model_dir
from ...utilities.core.serialization import json_ready
from ..provenance.datasets import DATASET_REFERENCE_REPORT_NAME, load_dataset_reference_report
    ...
    for dataset_name, entries in grouped_entries.items():
        for entry in entries:
            ...
            if not dataset_path or not model_name:
                continue
            adapter_path = _resolve_adapter_path_for_model_dir(Path(str(dataset_path)).expanduser() / str(model_name))
    ...
    *,
    ...
    if isinstance(raw_dataset_layouts, list):
        ...
        for raw_entry in raw_dataset_layouts:
            if not isinstance(raw_entry, Mapping):
                continue
            entry = {key: value for key, value in raw_entry.items() if key not in {"dataset_name", "sample_count"}}
    ...
    dataset_groups = (
        dataset_reference_payload.get("dataset_groups", []) if isinstance(dataset_reference_payload, Mapping) else []
    )
...
def build_public_spectral_metadata(
    *,
    ...
    public = {key: value for key, value in cleaned.items() if key not in _PUBLIC_METADATA_DROP_KEYS}
