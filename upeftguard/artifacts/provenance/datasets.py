...
def _dataset_name_and_path_for_item(*, model_dir: Path, dataset_root: Path) -> tuple[str, str]:
        ...
        for model_name, raw_entry in model_index.items():
            ...
            for key in [
                ...
                if left is not None and right is not None and left != right:
                    gaps.append(f"Conflicting {key} for model '{model_name}': {left!r} vs {right!r}")
            ...
            if left_label is None and right_label is not None:
                ...
                gaps.append(f"Conflicting label for model '{model_name}': {left_label!r} vs {right_label!r}")
    return merged, gaps
...
def _build_dataset_groups(model_index: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for (_dataset_path, _dataset_name), group in sorted(grouped.items(), key=lambda item: item[0][1]):
        ...
        group["model_family_counts"] = {key: int(value) for key, value in sorted(group["model_family_counts"].items())}
        group["attack_name_counts"] = {key: int(value) for key, value in sorted(group["attack_name_counts"].items())}
...
def _build_summary(
    *,
    ...
    for group in dataset_groups[:4]:
        ...
        if labels:
            preview_chunks.append(f"{group['dataset_name']} ({int(group['sample_count'])} models; labels {labels})")
        else:
    ...
    return f"{status} {len(dataset_groups)} dataset group(s) covering {coverage} models: " + "; ".join(preview_chunks)


def finalize_dataset_reference_payload(
    *,
    artifact_kind: str,
...
def _resolve_extraction_payload_from_run_dir(run_dir: Path) -> dict[str, Any]:
    for report_dir in (run_dir / "features", run_dir / "reports"):
        report_path = default_dataset_reference_report_path(report_dir)
    ...
    *,
    ...
    if isinstance(inputs, list) and inputs:
        ...
        return merge_dataset_reference_payloads(
            ...
            artifact_kind=kind,
            artifact_model_count=(int(artifact_model_count) if artifact_model_count is not None else None),
            source_artifacts=source_artifacts,
        ...
        if model_names_path is not None and model_names_path.exists():
            ...
            return _build_incomplete_payload_from_model_names(
                ...
                artifact_model_count=(
                    int(artifact_model_count) if artifact_model_count is not None else len(model_names)
                ),
                provenance_gaps=[
                    f"Could not load manifest provenance for merge report {report_path}; manifest_json={manifest_json}"
                ],
    ...
    return build_dataset_reference_payload_from_items(
        ...
        manifest_json=manifest_json,
        dataset_root=dataset_root,
        artifact_model_count=(int(artifact_model_count) if artifact_model_count is not None else len(items)),
